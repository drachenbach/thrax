...
#ifndef THRAX_RMSPROPPARAMETERUPDATER_H
#define THRAX_RMSPROPPARAMETERUPDATER_H
...
class RMSPropParameterUpdater: public ParameterUpdater {
public:
    RMSPropParameterUpdater(pt::ptree& hyperParameters, const ParameterMap& parameters): ParameterUpdater(hyperParameters, parameters) {
    ...
    virtual void calculateUpdate(GradientMap& gradients) {
        for (auto& gradient: gradients) {
            for (auto& ptr: gradient.second.getIdToCol()) {
                ...
                VectorXd d = (alpha * gradient.second.get(ptr.first).array() / (accumulatedGradients.at(gradient.first).col(ptr.first).array() + delta).sqrt()).matrix();
...
#endif //THRAX_RMSPROPPARAMETERUPDATER_H
